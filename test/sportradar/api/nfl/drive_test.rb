...
class Sportradar::Api::Content::DriveTest < Minitest::Test
  ...
  def test_it_initializes_a_drive
    data_object = Sportradar::Api::Content::Article.new(@attrs)
    assert [:response, :id, :sequence, :start_reason, :end_reason, :play_count, :duration, :first_downs, :gain, :penalty_yards, :scoring_drive, :quarter, :team, :plays, :events].all? { |e| data_object.attributes.include?(e) }
  end

  def test_plays_return_empty_array_when_nil
    article = Sportradar::Api::Content::Article.new(@attrs.merge("plays" => nil))
